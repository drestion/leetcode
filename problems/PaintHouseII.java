...
public class PaintHouseII {
  /*
   *    11:55am 0921 started
   *        12:02pm draft done.
   *        12:03pm 1pass
   *         #enter a general case of Paint House I. dp[i][c] = min(dp[i-1][nc]) + costs[i][c];
   *
   */
  public int minCostII(int[][] costs) {
   if (costs == null || costs.length < 1 || costs[0].length < 1) return 0;
   if (costs.length > 1 && costs[0].length < 2) return 0;

   int xlen = costs.length, clen = costs[0].length;
   int[][] dp = new int[xlen][clen];

   for (int c = 0; c < clen; c++) dp[0][c] = costs[0][c];
